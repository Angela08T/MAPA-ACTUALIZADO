...
const CapaRobos = ({ visible, filtros = null }) => {
  ...
  // Los eventos de conteo se manejan automáticamente en el hook useRobosQuery
    ...
    <>
      {(loading || isFetching) && (
        ...
        >
          ...
          >
            {loading ? 'Cargando robos...' : 'Actualizando...'}
          </span>
        </div>
      )}
      ...
      {error && (
        ...
        >
          Error cargando robos: {error.message}
        </div>
      )}

      <LayerGroup>
        {robosFiltrados.map((item, idx) => {
...
export default React.memo(CapaRobos);
