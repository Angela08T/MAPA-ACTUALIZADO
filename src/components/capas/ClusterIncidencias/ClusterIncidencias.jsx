...
const determinarTipo = (descripcion) => {
  ...
  const esExtorsion = desc.includes('extorsion') || 
...
const ClusterIncidencias = ({ visible, radioCluster = 50, filtros = null }) => {
    ...
    clustersData.forEach(cluster => {
      ...
      const popupContent = `
        <div style="font-size: 13px; max-width: 260px;">
          ...
          ${Object.entries(cluster.puntos.reduce((tipos, punto) => {
  ...
  useEffect(() => {
    ...
    fetch('/data/Robo_extorsion.json')
        ...
        if (filtros && Object.keys(filtros).length > 0) {
          ...
          datosFiltrados = datosFiltrados.filter(item => {
            return Object.entries(filtros).every(([campo, valor]) => {
              if (!valor || valor.trim() === "" || campo === "fechaInicio" || campo === "fechaFin") return true;
      ...
      .catch(error => {
        console.error('❌ Error cargando datos para clustering:', error);
...
export default ClusterIncidencias; 