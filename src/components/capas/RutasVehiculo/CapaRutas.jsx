...
const CapaRutas = ({ visible, onRutaCalculada }) => {
    ...
    const calculateRoute = async (markersToRoute) => {
        ...
        try {
            ...
            const coordinates = markersToRoute.map(marker => `${marker.position[1]},${marker.position[0]}`).join(';');
        ...
        <>
            {markers.map((marker, index) => (
                <Marker
                    ...
                    icon={
                        index === 0 ? startIcon : 
                        index === markers.length - 1 ? endIcon : 
                        intermediateIcon
                    }
                >
                    <Popup>
                        {index === 0 ? "Origen" : 
                         index === markers.length - 1 ? "Destino" : 
                         `Parada ${index}`}
                    </Popup>
                </Marker>
            ))}
