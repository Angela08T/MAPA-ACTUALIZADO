...
const CapaDefensaCivil = ({ visible }) => {
  ...
  useEffect(() => {
    ...
    fetch("/data/defensaCivil.json")
      ...
      .catch((err) => {
        console.error("❌ Error cargando defensaCivil.json:", err);
        setLugares([]);
        setGirosDisponibles([]);
      });
  }, [visible]);
  ...
  const lugaresFiltrados =
    filtroGiro.trim() === ""
      ? lugares
      : lugares.filter((item) =>
          normalizarTexto(item.GIro).includes(normalizarTexto(filtroGiro))
        );
      ...
      <LayerGroup>
        {lugaresFiltrados.map((item, idx) => {
          ...
          if (isNaN(lat) || isNaN(lng)) {
            console.warn(`⚠️ Coordenadas inválidas en índice ${idx}:`, item);
            return null;
          }
